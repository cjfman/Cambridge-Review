...
class Path(Element):
    def __init__(self, *, x1: float, y1: float, x2: float, y2: float, width=1, color='black', **kwargs):
...
class TickMark(Path):
    def __init__(self, *, x: float, y: float, height: float, width=1, color='black', **kwargs):
        y2 = y + height
        Path.__init__(self, x1=x, y1=y, x2=x, y2=y2, color=color, width=width)


class LinearGradient(Element):
    def __init__(self, gradient: Any, name=None):
        stops = []
